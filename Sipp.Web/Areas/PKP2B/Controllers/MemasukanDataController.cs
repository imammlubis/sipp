    ...
    public class MemasukanDataController : Controller
    {
        ...
        public async Task<string> EditServiceGeneralInformation(GeneralInformationViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                DataPkp2b dp = dataPkp2bRepository.Get().Where(c => c.CompanyID == model.ID).SingleOrDefault();
                dp.ID = dp.ID;
                ...
                var result = await dataPkp2bRepository.UpdateAsync(dp);
        ...
        [HttpPost]
        public JsonResult ListHistoryDataPkp2b(string id, [DataSourceRequest] DataSourceRequest request)
        {
        ...
        public async Task<ActionResult> Save(IEnumerable<HttpPostedFileBase> files, string id, string moduleType)
        {
            ...
            if (files != null)
            {
                ...
                {
                    case "1":
                        path = @"~/DocumentPkp2b/DataAkta/";
                        break;

                    case "2":
                        path = @"~/DocumentPkp2b/KronologiTahapan/";
                        break;
                    case "3":
                        path = @"~/DocumentPkp2b/DetailKronologiTahapan/";
                        break;
                    case "4":
                        path = @"~/DocumentPkp2b/Lingkungan/";
                        break;
                }
        ...
        public async Task<string> CreateServiceGeneralInformation(GeneralInformationViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                dp.CompanyID = company.ID;
                var result = await dataPkp2bRepository.AddAsync(dp);
